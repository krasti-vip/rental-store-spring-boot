...
public class BikeControllerRest {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<BikeDto> findByBicycleId(@PathVariable Integer id) {
        return bikeService
                .findById(id)
    ...
    public ResponseEntity<BikeDto> create(@RequestBody @Valid BikeDtoCreeate bikeDtoCreeate) {
        BikeDto createdBike = bikeService.create(bikeDtoCreeate);
    ...
    public ResponseEntity<List<BikeDto>> getBikeByUserId(
